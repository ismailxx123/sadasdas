...
client.unload = command => {
  ...
  if (msg.content.toLowerCase() === 'sa') {
    msg.reply('Aleyküm Selam,  Hoş Geldin , Nasılsın İyi Misin?');
  }
});
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'link') {
    msg.channel.send('discord.gg/DWS8V86Kxn SINIRSIZ DAVETIMIZ.');
  }
});
//////Gelen-giden
client.on('guildMemberAdd', member => {

  const channel = member.guild.channels.find('name', '💬┇ɢᴇɴᴇʟ-sᴏʜʙᴇᴛ');// 'notech-log' log ismidir. değiştirebilirsiniz. belirttiğiniz isme giriş çıkış gösterecektir.
  if (!channel) return;
  const embed = new Discord.RichEmbed()
  .setColor('0x00cc44')
  .setAuthor(client.user.username, client.user.avatarURL)
  .setThumbnail(member.user.avatarURL)
  .setTitle(`${member.user.username} Böyle Bir Orospu Cocuğu Sunucuya katıldı.`)
  .setTimestamp()
...
client.on('guildMemberRemove', member => {
  ...
  .setAuthor(client.user.username, client.user.avatarURL)
  .setThumbnail(member.user.avatarURL)
  .setTitle(`${member.user.username} Sonrada Ağlayıp Sunucudan ayrıldı.`)
  .setTimestamp()
  channel.sendEmbed(embed);
});
...
.then(channel =>
 ...
 message.guild.createChannel('❤fearless-giriş-çıkış', 'text', [{
  id: message.guild.id,
  deny: ['SEND_MESSAGES']
}])
.then(channel =>
...
client.on('guildMemberAdd', member => {
  ...
  const channel = member.guild.channels.find('name', 'sayaç');// 'notech-log' log ismidir. değiştirebilirsiniz. belirttiğiniz isme giriş çıkış gösterecektir.
  if (!channel) return;
  const embed = new Discord.RichEmbed()
  .setColor('0x00cc44')
  .setAuthor(client.user.username, client.user.avatarURL)
  .setThumbnail(member.user.avatarURL)
  .setTitle(`${member.user.username}\nAsena RolePlay Hoşgeldin.\nKayıt Olmak İçin Kayıt Beklemeye Geçip !kayıt Yazman Yeterlidir! `)
  .setTimestamp()
  channel.sendEmbed(embed);
});
...
client.on('message', async msg => {
   ...
   .setTitle(`:warning: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    .setColor('RANDOM'));
		  return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
     .setTitle('GKazım/DJ | Şarkı Kuyruğu')
